...
public class OAuth2Controller {
    ...
    public User registerKakaoUser(KakaoProfile profile){
        ...
        String name = profile.getKakaoAccount().getProfile().getNickname(); // 카카오에서 이름 가져오기
        ...
        // 회원가입 진행하기 전에
        // 이미 가잆한 회원인지, 혹은 비가징바징닟 체크한다.
        User user = userService.findByUsername(username); // 이름 찾기
    ...
    public void loginKakaoUser(User kakaoUser){
        ...
        // Authentication 수동으로 만들기
        Authentication authentication = authenticationManager.authenticate(authenticationToken);
        SecurityContext sc = SecurityContextHolder.getContext();
        sc.setAuthentication(authentication); // 인증된 authentication 추가..
