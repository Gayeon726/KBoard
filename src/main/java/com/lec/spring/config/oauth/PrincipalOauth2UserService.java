...
//    private PasswordEncoder passwordEncoder; // 회원 가입시 PW 인코딩
    ...
    // provider 로부터 받는 userRequest 데이터에 대한 후처리 진행
    @Override
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        System.out.println("PrincipalOauth2UserService.loadUser() 호출");
        ...
        if(user == null) { // 미가입자인 경우, 회원 가입 진행
            ...
            if(cnt > 0){
                ...
                user = userService.findByUsername(username); // 다시 읽어온다. regDate 정보 등.. 읽어본 정보가 세션에 들어가야하기 때문임
            }
        }else {
